...
def satellite():
    direc = os.path.join("processed_satellite_images/Delhi_July.png")
    with open(direc, "rb") as image_file:
        image = base64.b64encode(image_file.read())
    image = image.decode('utf-8')
    return render_template('satellite.html', data=data, image_file=image, months=months, text="Delhi in January 2023")
...
def satelliteimages():
    ...
    text = f"{place} in {date} 2023"

    direc = os.path.join("processed_satellite_images", f"{place}_{date}.png")
    with open(direc, "rb") as image_file:
...
def get_predicts():
    try:
        ...
        location = cityname
        api_key = 'dgv9xfEwGQUZ8XfDW9XVRNzhbwuVOGJ7VK_AMdrjayc' # Acquire from developer.here.com
        ...
        return render_template('predicts.html', cityname="Information about " + cityname, cities=cities, temp=round(final[0], 2), maxt=round(final[1], 2), wspd=round(final[2], 2), cloudcover=round(final[3], 2), percip=round(final[4], 2), humidity=round(final[5], 2), pred = pred,latitude=latitude, longitude=longitude)
    except:
